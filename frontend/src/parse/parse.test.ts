...
const hasSample = existsSync(resolve(SAMPLE_DIR, `${BASENAME}.vkx`));
...
// The private sample recording isn't committed to the repo (see .gitignore), so these
// suites only run where it's present (e.g. locally), not in CI.
if (hasSample) {
  describe('VKX parser', () => {
    const vkx = loadVkx();
  ...
  it.skip('sample-dependent parser tests skipped (01_Sample Data not present)', () => {});
}
